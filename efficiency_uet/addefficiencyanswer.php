...
<body>
<fieldset>
<legend>User Experience Test -> Efficiency -> Set User Score</legend>
<script>
    function doCheck(){
...
<form name="form1" action="addefficiencyanswer.php" method="POST" onsubmit="return doCheck()">
<p></p>
<table>
<tr>
<td>Project Name</td>
...
$sql = "SELECT DISTINCT project FROM user WHERE efficiency='No' AND project='$myrow1[name]'";
...
<form name="form2" action="addefficiencyanswer.php" method="POST" onsubmit="return doCheck2()">
<tr>
<td>Username</td>
...
$sql = "SELECT DISTINCT user FROM user WHERE efficiency='No' AND project='$_POST[pid]'";
$result = mysql_query($sql,$con);
if ($myrow = mysql_fetch_array($result)){
do {
...
$sql4 = "SELECT * FROM assignefficiency WHERE project='$_POST[pj]'";
$result4 = mysql_query($sql4,$con);

if ($myrow4 = mysql_fetch_array($result4)){

?>
<form name="form" action="addefficiencyanswer.php" method="POST">
...
$sql7 = "SELECT * FROM assignefficiency WHERE project='$_POST[pj]'";
...
$sql1 = "SELECT * FROM efficiency WHERE no='$myrow7[no]' ORDER BY CAST(no AS UNSIGNED) ASC";
$result1 = mysql_query($sql1,$con);
if ($myrow1 = mysql_fetch_array($result1)){
...
<tr>
<td align="center"><input type="submit" name="submitend" value="SUBMIT"><td> 
</tr>
</table>
</form>
<?php
	...
	echo "<p style=\"color:red\">No efficiency question have been assigned!</p>";
}
mysql_close($con);
}else{}
?>
...
$sql1 = "SELECT * FROM assignefficiency WHERE project='$_POST[project]' ORDER BY CAST(no AS UNSIGNED) ASC";
$result1 = mysql_query($sql1,$con);

if ($myrow1 = mysql_fetch_array($result1)){
do {
$MyName = $myrow["no"] . $myrow1["no"];
$sql2="INSERT INTO answerefficiency (project,name,task,no,answer)
VALUES
('$_POST[project]','$_POST[username]','$myrow[no]','$myrow1[no]','$_POST[$MyName]')";
...
$sql="UPDATE user SET efficiency='Yes' WHERE user='$_POST[username]'";
...
?>
<script type="text/javascript">
alert("For project '<?php echo $_POST[project] ?>', '<?php echo $_POST[username] ?>' efficiency score has been added successfully!");
document.location.href='addefficiencyanswer.php'
</script>
<?php
